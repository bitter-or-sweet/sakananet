...
RSpec.describe User, type: :model do
  describe 'バリデーションチェック' do
    ...
    it 'password_confirmationが入力されていない場合、バリデーションが機能してinvalidになるか' do
      user = build(:user, password_confirmation: nil)
      expect(user).to be_invalid
      expect(user.errors.full_messages).to include("パスワード確認を入力してください")
    end

    it 'nameが入力されていない場合、バリデーションが機能してinvalidになるか' do
      user = build(:user, name: nil)
      expect(user).to be_invalid
      expect(user.errors.full_messages).to include("名前を入力してください")
    end

    it 'nemeが11文字以上の場合、バリデーションが機能してinvalidになるか' do
      user = build(:user, name: "aaabbbcccddd")
      expect(user).to be_invalid
      expect(user.errors.full_messages).to include("名前は10文字以内で入力してください")
    end

    it 'emailが入力されていない場合、バリデーションが機能してinvalidになるか' do
      user = build(:user, email: nil)
      expect(user).to be_invalid
      expect(user.errors.full_messages).to include("メールアドレスを入力してください")
    end

    it 'emailが重複した場合、バリデーションが機能してinvalidになるか' do
